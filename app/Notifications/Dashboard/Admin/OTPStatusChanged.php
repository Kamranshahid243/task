...
namespace App\Notifications\Dashboard\Admin;
...
class OTPStatusChanged extends Notification
{
    use Queueable;
    private $message;
    private $icon;
    ...
    {
        if( $status == 'enable' ){
            $this->message          = 'You enabled OTP (One Time Password)';
            $this->icon             = 'fa fa-check text-success';
            $this->timelineIcon     = 'fa fa-check bg-green';
        }
        else{
            $this->message          = 'You disabled OTP (One Time Password)';
