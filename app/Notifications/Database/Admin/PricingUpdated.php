...
class PricingUpdated extends Notification
{
    use Queueable;
    private $pricings;
    private $message;
    ...
    public function __construct( $type, $pricings, $superadmin, $nofityType )
    {
        ...
        if( $nofityType == 'self-notify' ){
            ...
            $this->message          = 'You updated '.$type.' pricing';
            $this->icon             = 'fa fa-dollar text-yellow';
            $this->timelineIcon     = 'fa fa-dollar bg-yellow';
           
        }
        else{
            $this->message          = '<a href="/admin/users/management/'.$superadmin->id.'">'.$superadmin->first_name.' '.$superadmin->last_name.'</a> updated '.$type.' pricing';
            $this->icon             = 'fa fa-dollar text-yellow';
            $this->timelineIcon     = 'fa fa-dollar bg-yellow';
        }
