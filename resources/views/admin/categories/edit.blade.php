    ...
    <div class="row">
        <div class="col-md-4">
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-verticle form-with-attachment optional-attachment" method="post" action="{{ url('/admin/categories/management/'.$found->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="box-body">
                        @if( $found->thumbnail )
					...
					<tbody>
                        @if( isset( $categories ) and $categories->count() )
                            @foreach( $categories as $category )
                                <tr>
                                    <td><img class="thumb-square-50x50" src="{{ asset( $category->thumbnail ) }}" ></td>
                                    <td>{{ $category->name }}</td>
                                    <td>
