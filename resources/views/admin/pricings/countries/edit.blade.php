...
<div class="content-wrapper" style="min-height: 1126px;">
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">Edit Country Level Pricing</h3>
                </div>
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-verticle form" method="post" action="{{ url('/admin/countries/pricings/'.$editablePricing->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="box-body">
                        <div class="form-group">
                            <label class="">Select one or more countries<span class="text-red">*</span></label>
                            <select class="form-control select2" name="country_id" placeholder="safas">
                                @if( $countries->count() )
                                    @foreach( $countries as $country)
                                        @if( $editablePricing->name == $country->name)
                            ...
                            <span><em>You can select multiple countries to set the same price</em></span>
                        </div>
                        <div class="form-group">
                            <label class="">Price Per day<span class="text-red">*</span></label>
                            <input type="number" class="form-control" name="price_per_day" value="{{ $editablePricing->price }}" placeholder="Price Per Day">
                        </div>
                    </div>
                    <!-- /.box-body -->
        ...
        <div class="col-md-8">
            <div class="box box-primary ">
                <div class="box-header with-border">
                    <h3 class="box-title">View All Countries Pricings</h3>
                </div>
                <div class="box-body">
                        ...
                        @if( isset( $pricings ) and $pricings->count() )
                            @foreach( $pricings as $pricing )
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('/admin/countries/pricings/'.$pricing->id.'/edit') }}"  title="Edit Admin Account" data-action="edit" class="btn btn-xs btn-default"><i class="fa fa-edit"></i></a> 
                                        <a href="#" data-type="admin" data-id="{{ $pricing->id }}" data-url="/admin/countries/pricings/{{$pricing->id}}"  title="Delete"data-action="delete" class="custom-table-btn btn btn-xs btn-default"><i class="fa fa-trash-o"></i></a>
