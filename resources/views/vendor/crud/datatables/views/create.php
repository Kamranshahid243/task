    ...
    <section class="content">
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Create <?= $gen->titleSingular() ?></h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form class="form-verticle form" method="post" action="{{ url('/admin/<?= $gen->route() ?>') }}">
                @csrf
...
<?php
$defaultAttrs = 'class="form-control"';
$placeholder = 'placeholder="' . ucwords(str_replace("_", " ", $field->name)) . '"';
?>
<?php if (!\Nvd\Crud\Db::isGuarded($field->name)) { ?>
        <label><?= ucwords(str_replace("_", " ", $field->name)) ?><?php if ($field->required) {?><span class="text-red">*</span><?php } ?></label>
